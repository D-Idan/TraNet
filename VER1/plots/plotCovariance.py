...
def plot_error_evolution_trace(trace_EKF, trace_knet, emp_error_EKF, emp_error_knet, T, path, mismatch=False):
    ...
    trace_EKF = trace_EKF.cpu().numpy()
    trace_knet = trace_knet.cpu().numpy()
    ...
    ax[0].plot(t, db(trace_EKF), '--b', label="TransNet Covariance Trace")
    ax[0].plot(t, db(emp_error_knet), '-r', label="KalmanNet Empirical Error")
    ax[0].plot(t, db(emp_error_EKF), '-b', label="TransNet Empirical Error")


    ax[1].plot(t, db(emp_error_knet - trace_knet), '-r', label="KalmanNet Error Deviation")
    ax[1].plot(t, db(emp_error_EKF - trace_EKF), '-b', label="TransNet Error Deviation")
    ...
    if mismatch:
        ...
        plt.savefig(Path.joinpath(path, Path("error_evolution_short.png")), dpi=300)
    else:
