...
def TransNNTest(SysModel, test_input, test_target, path_results, nclt=False, rnn=False, IC=None, calculate_covariance=False):

    path_res = Path.joinpath(Path('.').absolute().parent , Path(path_results))
    try:
        try:
            Model = torch.load(Path.joinpath(path_res,Path('best-model.pt')))
            path_results = path_res
        except:
            first = Path.joinpath(Path().absolute(), Path('KalmanNet/ERRCOV_ICASSP22').absolute())
            path_res = Path.joinpath(first, Path(path_results)).absolute()
            Model = torch.load(Path.joinpath(path_res, Path('best-model.pt')))
            path_results = path_res
    except:
        path_results = Path.joinpath(Path('.').absolute() , Path(path_results))
    ...
    if(rnn):
        ...
        Model = torch.load(Path.joinpath(path_results,Path('best-model.pt')))
