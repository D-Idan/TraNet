...
from data.Lorenz_Atractor.model import getJacobian
...
class KalmanTransformer(torch.nn.Module):
    ...
    def __init__(self,d_model_enc, d_model_dec, in_mem_t=in_mem_t, out_mem_t=out_mem_t):
        ...
        fcMaxMem = nn.Linear(6 * max_mem_len , 32).to(dev) # 72 - > 32
        fc2KG = nn.Linear(32 , self.m * self.n).to(dev)  # 32 - > 6(KG)
        self.linearTrans_Out2KG = nn.Sequential(flatten, fcMaxMem, dropout01, nn.ELU(), fc2KG).to(dev)
    ...
    def KGain_step(self, obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff):
        ...
        net = self.linearTrans_Out2KG.to(dev)
        net_output = net(trans_out_padded)
        return net_output
