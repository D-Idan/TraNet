...
class ExtendedKalmanFilter:
    ...
    def Predict(self):
        ...
        self.m1x_prior = self.f(self.m1x_posterior.to(dev))
        # Compute the Jacobians
        #self.UpdateJacobians(getJacobian(self.m1x_posterior,self.f), getJacobian(self.m1x_prior, self.h))
        self.UpdateJacobians(getJacobian(self.m1x_posterior,self.f), getJacobian(self.m1x_prior, self.h))
        # Predict the 2-nd moment of x
        self.m2x_prior = torch.matmul(self.F.to(dev), self.m2x_posterior.to(dev))
        self.m2x_prior = torch.matmul(self.m2x_prior.to(dev), self.F_T.to(dev)) + self.Q.to(dev)

        # Predict the 1-st moment of y
        self.m1y = self.h(self.m1x_prior)
        # Predict the 2-nd moment of y
        self.m2y = torch.matmul(self.H, self.m2x_prior.to(dev))
        self.m2y = torch.matmul(self.m2y, self.H_T) + self.R

    # Compute the Kalman Gain
    def KGain(self):
        self.KG = torch.matmul(self.m2x_prior.to(dev), self.H_T)
    ...
    def Correct(self):
        ...
        self.m2x_posterior = self.m2x_prior.to(dev) - torch.matmul(self.KG, self.m2x_posterior)
