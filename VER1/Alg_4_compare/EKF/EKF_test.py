...
from Alg_4_compare.EKF.EKF import ExtendedKalmanFilter
...
def EKFTest(SysModel, test_input, test_target, modelKnowledge = 'full', allStates=True, init_cond=None, calculate_covariance=False):
    ...
    for j in range(0, N_T):
        ...
        EKF.GenerateSequence(test_input[j, :, :].to(device))

        if(allStates):
            MSE_EKF_linear_arr[j] = loss_fn(EKF.x, test_target[j, :, :].to(device)).item()
        else:
