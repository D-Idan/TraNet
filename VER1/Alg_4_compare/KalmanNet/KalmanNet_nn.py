...
# parameters controlling the size of the input and output FC layers
in_mult = 5
out_mult = 40
...
class KalmanNetNN(torch.nn.Module):
    ...
    def InitKGainNet(self, prior_Q, prior_Sigma, prior_S):
        ...
        # initializing priors on noise for the initial state of the GRUs
    ...
    ### Initialize Sequence ###
    ###########################
    def InitSequence(self, M1_0, M2_0, T, calculate_covariance=False):
    ...
    def KGain_step(self, obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff):
        ...
        # FC 5
        in_FC5 = fw_evol_diff
        out_FC5 = self.FC5.to(dev)(in_FC5)

        # Q-GRU
        in_Q = out_FC5
        out_Q, self.h_Q = self.GRU_Q.to(dev)(in_Q, self.h_Q.to(dev))

        # FC 6
        in_FC6 = fw_update_diff
        out_FC6 = self.FC6.to(dev)(in_FC6)

        # Sigma_GRU
        in_Sigma = torch.cat((out_Q, out_FC6), 2)
        out_Sigma, self.h_Sigma = self.GRU_Sigma.to(dev)(in_Sigma, self.h_Sigma.to(dev))

        # FC 1
        in_FC1 = out_Sigma
        out_FC1 = self.FC1.to(dev)(in_FC1)

        # FC 7
        in_FC7 = torch.cat((obs_diff, obs_innov_diff), 2)
        out_FC7 = self.FC7.to(dev)(in_FC7)


        # S-GRU
        in_S = torch.cat((out_FC1, out_FC7), 2)
        out_S, self.h_S = self.GRU_S.to(dev)(in_S, self.h_S.to(dev))


        # FC 2
        in_FC2 = torch.cat((out_Sigma, out_S), 2)
        out_FC2 = self.FC2.to(dev)(in_FC2)
        ...
        out_FC3 = self.FC3.to(dev)(in_FC3)

        # FC 4
        in_FC4 = torch.cat((out_Sigma, out_FC3), 2)
        out_FC4 = self.FC4.to(dev)(in_FC4)
    ...
    def init_hidden(self):
        # setting the initial state of the GRUs
