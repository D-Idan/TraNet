import math
from data.KalmanNet_data import DataGen, DecimateData, Decimate_and_perturbate_Data
import Alg_4_compare.KalmanNet.KalmanNet_sysmdl
...
cur = pathlib.Path().absolute().parents[0]
ERRCOVpath_model = pathlib.Path.joinpath(cur,
                                         pathlib.Path(path_model)).absolute()
cur = pathlib.Path().absolute()
loreAtt = pathlib.Path.joinpath(cur,'KalmanNet','ERRCOV_ICASSP22','Simulations','Lorenz_Atractor')
pathsList = [loreAtt,ERRCOVpath_model, pathlib.Path(path_model).absolute()]
...
def load_data(obs_model, noise, process_noise=None, discrete=False, randomize_init_conditions=True):
    ...
    parent_dir = pathlib.Path().absolute().parents[0]
    path_base = pathlib.Path.joinpath(parent_dir,path_base)
    path_base = pathlib.Path(r'C:\Users\Idan Daniel\Desktop\לימודים\תואר שני\Deep_Learning\final_project\KalmanNet\ERRCOV_ICASSP22\Simulations\Lorenz_Atractor\data\continuous_randomized\q_none')
    os.makedirs(path_base, exist_ok=True)

    IC_path = pathlib.Path.joinpath(path_base , pathlib.Path("IC.pt"))
    obs_path = pathlib.Path.joinpath(path_base , f"obs_{obs_model}_{str(noise)}.pt")
    
    if discrete:
        GT_path = path_base + "GT.pt"
    else:
        GT_test_path = pathlib.Path.joinpath(path_base , "GT_test.pt")
        GT_test_long_path = pathlib.Path.joinpath(path_base , "GT_test_long.pt")
        GT_test_short_path = pathlib.Path.joinpath(path_base , "GT_test_short.pt")
        GT_CV_path = pathlib.Path.joinpath(path_base , "GT_CV.pt")
        GT_train_path = pathlib.Path.joinpath(path_base , "GT_train.pt")
        GT_undecimated_path = pathlib.Path.joinpath(path_base , "GT_undecimated.pt")
    ...
    if discrete:
        ...
        try:
            ...
            try: 
                ...
                # testing = gen_decimated(2, N_test, 1, sys_model_test, IC_test, GT_test_undecimated)
                threads[0] = Process(target=gen_decimated, args=(0, N_test_long, 1, sys_model_test_long, IC_test_long, GT_test_long_undecimated,))
                threads[1] = Process(target=gen_decimated, args=(1, N_test_short, 1, sys_model_test_short, IC_test_short, GT_test_short_undecimated,))
                threads[2] = Process(target=gen_decimated, args=(2, N_test, 1, sys_model_test, IC_test, GT_test_undecimated,))
                threads[3] = Process(target=gen_decimated, args=(3, N_CV, 1, sys_model_CV, IC_CV, GT_CV_undecimated,))
                ...
                for t in range(4):
                    threads[t] = Process(target=gen_decimated, args=(t, N_train, 4, sys_model_train, IC_train, GT_train_undecimated,))
    ...
    if not discrete:
        ...
        try:
            ...
            for t in range(N_threads):
                threads[t] = Process(target=generate_trajectory, args=(t, N_trajectories, N_threads, GT, GT_undecimated, Q, randomize_init_conditions,))
                threads[t].start()
            for thread in threads:
                thread.join()
...
if __name__ == "__main__":
    load_data("identity", 0, process_noise=None)
