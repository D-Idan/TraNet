...
from data.Lorenz_Atractor.parameters import m, n, J, delta_t, delta_t_gen, H_design, B, C, B_mod, C_mod, delta_t_mod, J_mod, H_mod, H_design_inv, H_mod_inv, H_rotated, J_inacc
...
def h(x):
    ...
    return torch.matmul(H_design.to(dev),x)
...
def f_interpolate(x, n=2):
    ...
    for _ in range(n):
        try:
            A = torch.add(torch.reshape(torch.matmul(B_mod, x.to(dev)),(m,m)).T,C_mod)#.to(dev, non_blocking=True)
        except:
            A = torch.add(torch.reshape(torch.matmul(B_mod, x.to('cpu')),(m,m)).T,C_mod)#.to(dev, non_blocking=True)

        # Taylor Expansion for F    
        F = torch.eye(m)#.to(dev, non_blocking=True)
        for j in range(1,J+1):
            F_add = torch.matrix_power(A*delta_t/n, j)/math.factorial(j)
            F = torch.add(F, F_add.to(F.device))
        x = torch.matmul(F, x.to(F.device))
        return x
