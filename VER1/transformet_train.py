...
# cur = pathlib.Path().parent
# ERRCOVpath_model = pathlib.Path.joinpath(cur, 'KalmanNet', 'ERRCOV_ICASSP22',
#                                          pathlib.Path(path_model)).absolute()
# if ERRCOVpath_model not in sys.path:
#     sys.path.insert(1, str(ERRCOVpath_model))
...
def NNTrain(SysModel, Model, cv_input, cv_target, train_input, train_target, path_results, nclt=False,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=50, T_mult=3, eta_min=1e-15, last_epoch=-1)
    ...
    for ti in range(0, N):
        ...
        for j in range(0, N_CV):
            ...
            # Choose Different memory size
            mem_size_rand = torch.randint(low=1,high=12, size=[1]).item()
            Model.InitSequence(init_conditions, SysModel.m2x_0, SysModel.T, mem_t=12)
            ...
            if (nclt):
                ...
                MSE_cv_linear_batch[j] = loss_fn(x_Net_cv[:, :outputs], cv_target[j, :, :outputs]).item()
        ...
        if (MSE_cv_dB_epoch[ti] < MSE_cv_dB_opt):
            ...
            if (rnn):
                ...
                torch.save(Model, pathlib.Path.joinpath(path_results , 'best-model.pt'))
        ...
        for j in range(0, N_B):
            ...
            # Choose Different memory size
            mem_size_rand = torch.randint(low=4, high=8, size=[1]).item()
            Model.InitSequence(init_conditions, SysModel.m2x_0, SysModel.T, mem_t=12)
            ...
            if (nclt):
                ...
                LOSS = loss_fn(x_Net_training[:,:outputs], train_target[n_e, :, :outputs])
        ...
        # torch.nn.utils.clip_grad_norm_(Model.parameters(), max_norm=0.004, norm_type=2)
        # torch.nn.utils.clip_grad_value_(Model.parameters(), clip_value=0.00004)
